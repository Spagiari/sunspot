    ...
    int DetectBlobs(IplImage* frame, SolarAnalyzer::blob_collection & blobs)
    {
        ...
        for(int row = 0; row < frame->height; ++row)
        {
            for(int column = 0; column < frame->width; ++column)
            {
                ...
                if(byte == 255)
                {
                    ...
                    int stop = column-1;
                    lineBlob lineBlobData = {static_cast<unsigned int>(start), static_cast<unsigned int>(stop), static_cast<unsigned int>(blobCounter), false};
        ...
        for(int row = 0; row < imgData.size(); ++row)
        {
            for(int entry = 0; entry < imgData[row].size(); ++entry)
            {
                ...
                {
                    blob blobData(coordinate(imgData[row][entry].min, row),coordinate(imgData[row][entry].max, row));
        ...
        return SolarAnalyzer::SAReturn::E_OK;
    }
