...
namespace SolarAnalyzer
{
    ...
    IplImage *threshold(IplImage *img, int debug)
    {
    ...
    IplImage *detect_sunspots(IplImage *img, int debug)
    {
        ...
        do {
            ...
            cvSaveImage("CV_MOP_BLACKHAT.jpg",dst);
            //printf("SE_SIZE = %d\n", se_size);
            cvReleaseStructuringElement(&se);
            
            IplImage *tmp = threshold(dst, debug);
            cvReleaseImage(&dst);
            dst = tmp;
